...
class Material(BaseModel):
    ...
    def from_data(
        ...
        """
        def _normalize_formula_source(v):
            if isinstance(v, str):
                return v.strip() or None
            if isinstance(v, (list, tuple, set)):
                for item in v:
                    out = _normalize_formula_source(item)
                    if out:
                        return out
                return None
            if v is None:
        ...
        # Be tolerant to malformed meta payloads from model output
        # (e.g., meta as list[dict] instead of dict).
        ...
        if not formula_source:
            formula_source = meta.get("formula source")
        formula_source = _normalize_formula_source(formula_source)
        chemical_formula, is_general = format_chemical_formula(
            meta.get("chemical formula")
        )

        return cls(
            name=format_chemical_name(meta.get("name")),
            symbol=format_chemical_name(meta.get("symbol")),
