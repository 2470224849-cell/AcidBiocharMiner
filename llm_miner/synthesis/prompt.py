...
For acid-modified biochar papers, map pyrolysis/carbonization to "heat_treatment", acid soaking/acidification to "chemical_synthesis", and rinse/neutralization to "washing".
Use recall-first target scope for acid-modified biochar adsorption/removal in water with unrestricted pollutants. If paragraph has quantitative preparation cues, keep candidate processes instead of returning empty.
...
PROMPT_STRUCT = """Extract the relevant information about the synthesis of materials mentioned in the paragraph.

There are guidelines for output format:
- Output must be in the format {{"meta":{{"name":"", "symbol":"", "chemical formula":""}}, "processes": list, "yield": str, "other properties": [str, str, ...], "biochar modification": [{{...}}]}}.
...
- For acid-modified biochar studies, if available, populate "biochar modification" with exact keys: "filename", "sample_id", "biomass_source", "pyrolysis_temp_C", "hold_duration_h", "heating_rate_C_min", "acid_type", "acid_conc_mol_L", "acid_time_h", "acid_temp_C", "modification_sequence", "SSA_m2_g", "APS_nm", "TPV_cm3_g", "ash_percent", "C_percent", "O_percent", "N_percent", "H_percent", "pH_pzc". Keep missing values as "".
- Do not create keys with numeric suffixes like "_1", "_2" for "biochar modification".
- In the three-source framework (Table/Text/Graph), this synthesis pathway primarily serves the Materials table (sheet1). Focus on preparation/acid-modification records and avoid generating adsorption rows here unless explicitly described with all required adsorption fields.
- Prioritize acid-modified biochar rows; for borderline cases with quantitative preparation cues, keep candidate rows and leave missing fields as "".
- For sample-level biochar fields (pyrolysis_temp_C, hold_duration_h, heating_rate_C_min, acid_conc_mol_L, acid_time_h, acid_temp_C), output a single value per sample row. Never output comma-separated lists/ranges for one sample.
- If the paper reports a screening series (e.g., multiple pyrolysis temperatures) and then states one selected condition for subsequent preparation/experiments, use only that selected single value for derived modified samples.
- If a unique value cannot be linked to the sample with explicit evidence, keep the field as "".
- Do not output purely narrative synthesis claims without quantitative evidence.
