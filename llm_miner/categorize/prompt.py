...
- You should return ["property"] when the paragraph includes sample-level quantitative property/adsorption information. Units are preferred but not mandatory if values and context are clear.
- In acid-modified biochar papers, treat preparation details (biomass source, pyrolysis temperature/time, heating rate, acid type/concentration/time/temperature, modification sequence) as "synthesis condition".
- In acid-modified biochar papers, treat adsorption and characterization metrics (SSA, APS, PV, elemental ratio, pHpzc, C0, Qe, pH, temperature, contact time, SLR) as "property" when numeric values are present.
- Target scope is recall-first: prioritize acid-modified biochar used for adsorption/removal in water, with pollutants not restricted to heavy metals, and include borderline numeric paragraphs as "property" rather than "else".
- If paragraph is clearly outside scope (no acid-biochar relation or no relevant quantitative data), classify as ["else"].
- If composite materials are dominant and biochar is only auxiliary, classify as ["else"] unless the paragraph explicitly reports acid-modified-biochar sample-level data.
- In the Table/Text/Graph framework, figure captions or figure-related paragraphs containing isotherm/curve numeric metrics (Qe, C0, pH, T, time, dosage) must be treated as "property", not "else".
- If there are both "synthesis condition" and "property", return ["property", "synthesis condition"].
- If the paragraph includes neither synthesis-condition information nor property information, return ["else"].
- A paragraph can be only ["property"] or only ["synthesis condition"]; it does not need to contain both.
...
FT_CATEGORIZE = (
    ...
    "Apply strict scope: acid-modified biochar for adsorption/removal in water, with pollutants not restricted to heavy metals. "
    "For acid-biochar work, treat figure/isotherm numeric metrics as \"property\". "
